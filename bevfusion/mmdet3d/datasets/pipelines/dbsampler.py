...
from .utils import box_collision_test, frustum_collision_test, get_frustum
...
class UnifiedDataBaseSampler(object):
    ...
    def sample_all(self, gt_bboxes, gt_labels, with_img=False):
        """Sampling all categories of bboxes for a scene.
        ...
        """
        # How many objects can be sampled for each class given the maximum number of objects of that class
        # allowed and the number of objects already in the scene
        sampled_num_dict = {}
        ...
        for class_name, sampled_num in zip(self.sample_classes,
            ...
            if sampled_num > 0:
                ...
                if len(sampled_cls) > 0:
                    ...
                    # Add sampled boxes to the list of boxes to avoid collisions with them in the future
        ...
        if len(sampled) > 0:
            ...
            for info in sampled:
                file_path = os.path.join(
                    self.dataset_root,
                    info['path']) if self.dataset_root else info['path']
                ...
                if with_img:
                    s_img = {}
                    for cam_type, patch_path in info['image_patch_paths'].items():
                        s_img[cam_type] = mmcv.imread(patch_path, 'unchanged')[..., ::-1]
    ...
    def sample_class_v2(self, name, num, gt_bboxes):
        ...
        """
        samples = self.sampler_dict[name].sample(num)
        samples = copy.deepcopy(samples)
        num_gt = gt_bboxes.shape[0]
        num_sp = len(samples)

        gt_frustums = get_frustum(gt_bboxes)
        gt_bboxes_bv = box_np_ops.center_to_corner_box2d(
            gt_bboxes[:, 0:2], gt_bboxes[:, 3:5], gt_bboxes[:, 6])

        sp_boxes = np.stack([sample['box3d_lidar'] for sample in samples], axis=0)
        sp_frustums = get_frustum(sp_boxes)
        sp_boxes_bv = box_np_ops.center_to_corner_box2d(
            sp_boxes[:, 0:2], sp_boxes[:, 3:5], sp_boxes[:, 6])
        
        all_bboxes_bv = np.concatenate([gt_bboxes_bv, sp_boxes_bv], axis=0)
        box_coll_mat = box_collision_test(all_bboxes_bv, all_bboxes_bv)
        frustum_coll_mat = frustum_collision_test(gt_frustums, sp_frustums)
        
        coll_mat = np.logical_or(box_coll_mat, frustum_coll_mat)
        diag = np.arange(all_bboxes_bv.shape[0])
        coll_mat[diag, diag] = False

        valid_samples = []
        for i in range(num_gt, num_gt + num_sp):
            if coll_mat[i].any():
                coll_mat[i] = False
                coll_mat[:, i] = False
            else:
                valid_samples.append(samples[i - num_gt])
...
class DataBaseSampler:
    ...
    Loads ground truth database.
    Create class-specific BatchSampler.
    ...
    Args:
        ...
        rate (float): Rate of actual sampled over maximum sampled number.
        prepare (dict): Name of preparation functions and the input value.
        sample_groups (dict): Sampled classes and numbers. Refers to oversampling.
        classes (list[str]): List of classes. Default: None.
        points_loader(dict): Config of points loader. Default: dict(
            type='LoadPointsFromFile', load_dim=4, use_dim=[0,1,2,3])
    """
    ...
    def __init__(
        ...
        # db_infos: Dict[class, List[sample_info]]]
        # sample_info: Dict[Union[name, path, image_idx, gt_idx, box3d_lidar, num_points_in_gt, difficulty, group_id], Any]
        self.db_infos = db_infos

        # load sample groups
        # TODO: more elegant way to load sample groups
        # sample_groups: List[Dict[class, int]]
        self.sample_groups = []
        for name, num in sample_groups.items():
            self.sample_groups.append({name: int(num)})
