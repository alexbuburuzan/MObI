...
def create_groundtruth_database(
    ...
    if db_info_save_path is None:
        db_info_save_path = osp.join(data_path,
                                     f"{info_prefix}_dbinfos_{split}.pkl")
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        point_indices = box_np_ops.points_in_rbbox(
            points, gt_boxes_3d, origin=(0.5, 0.5, 0.5)
        )
        group_dict = dict()
        ...
        if with_mask:
            ...
            all_image_patches = [{} for _ in range(num_obj)]
            ...
            for _idx, (_img, _lidar2image, cam_type) in enumerate(zip(imgs, lidar2image, cam_types)):
                ...
                visible = ((bboxes_2d[:, 2:] - bboxes_2d[:, :2]) > 1).all(axis=-1)
                ...
                object_img_patches = crop_image_patch_no_mask(
                    bboxes_2d, _img
                )

                for i, object_img_patch in enumerate(object_img_patches):
                    obj = org_indices[i]
                    all_image_patches[obj][cam_type] = object_img_patch
        ...
        for i in range(num_obj):
            ...
            image_patch_paths = {}
            if with_img:
                for cam_type, object_img_patch in all_image_patches[i].items():
                    image_patch_path = abs_filepath + f"_{cam_type}.png"
                    mmcv.imwrite(object_img_patch[..., ::-1], image_patch_path)
                    image_patch_paths[cam_type] = image_patch_path
            ...
            with open(abs_filepath + ".bin", "w") as f:
                gt_points.tofile(f)

            if (used_classes is None) or names[i] in used_classes:
                db_info = {
                    "name": names[i],
                    "path": rel_filepath + ".bin",
                    "image_idx": image_idx,
                    "gt_idx": i,
                    "box3d_lidar": gt_boxes_3d[i],
                    "num_points_in_gt": gt_points.shape[0],
                    "difficulty": difficulty[i],
                    "image_patch_paths": image_patch_paths,
